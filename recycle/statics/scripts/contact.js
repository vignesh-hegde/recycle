bueger_toggle = false; 
function bueger(){
    temp = [['0px',"45","0","#ffff"],["30px","0","-100%","rgba(0,0,0,0)"]]
...
window.onscroll = function(){
    ...
    if(top>=70){
        ...
        for(i=0;i<hov.length;i++){
            hov[i].style.color=style.getPropertyValue('--color-light')
        }
    }
}


var fact = document.getElementsByClassName("temp")
var i = 0
slideShow(fact);
function slideShow(){
    for(j=0;j<fact.length;j++)
    {
        fact[j].style.display = "none";
    }
    if(i<fact.length-1){
        i++;
    }else{
